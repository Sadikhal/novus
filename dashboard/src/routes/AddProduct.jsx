...
const getProductSchema = (isAdmin) => z.object({
  name: z.string().min(10, { message: "Product name must be at least 10 characters!" }).max(70, { message: "Product name must be at most 70 characters!" }),
  brandId: isAdmin
  ...
  ).optional().transform(arr => arr?.filter(f => f.name && f.detail) || []),
});
...
const AddProduct = () => {
  ...
  useEffect(() => {
    ...
    loadProduct();
  }, [id, fetchProduct, reset, toast, setUploadedImages]);
    ...
    if (fieldName === "size") {
      if (value === "Custom") {
        ...
        newValues = currentValues.includes(value)
          ...
          : [...currentValues.filter(item => item !== "Custom"), value];
  ...
  const handleEditorImageUploadComplete = useCallback((url) => {
    ...
    setUploadQueue(prev => prev.slice(1)); // Remove the processed editor image
  }, [setUploadQueue]);
  ...
  const handleEditorFileSelect = useCallback((e) => {
    ...
    setUploadFor('editor');
    setUploadQueue([files[0]]); // Only process one editor image at a time
    setShowCropModal(true);
  }, [setUploadQueue, setShowCropModal]);
  ...
  const onSubmit = async (formData) => {
    try {
      ...
      const finalSizes = Array.from(new Set(allSizes.map(s => s.toString().trim().toUpperCase())));
          ...
          <ImageCropModal
            ...
            onClose={() => {
              ...
              if (editorFileInputRef.current) {
                try { editorFileInputRef.current.value = null; } catch (err) {}
              }
            }}
            onUploadComplete={uploadFor === 'editor' ? handleEditorImageUploadComplete : handleImageUploadComplete}
