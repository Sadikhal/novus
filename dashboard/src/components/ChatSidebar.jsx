...
import { ChatListSkeleton } from "./ui/Loaders";
...
const ChatSidebar = ({ 
  ...
  const chatRole = targetRole === 'user' ? "customer" : targetRole;
  return (
    <div className={cn(
      ...
      <div className='w-full rounded-xl transition-all'>
        ...
        <ScrollArea className='w-full flex flex-col py-4 h-[400px] text-black'>
          {chatsLoading ? (
            ...
            chats.map((chat) => (
              <Link 
                to={`/${role}/${chatRole}-messages/${chat._id}`} 
                key={chat._id} 
                className={cn(
                  ...
                  'rounded-tr-xl border-b border-slate-200 pl-2 relative',
