...
const NotificationItem = ({ notification, onItemClick, closePopover, currentUser }) => {
  ...
  const handleClick = (e) => {
    ...
    if (conversationId) {
      const senderRole = sender?.role === "user" ? "customer" : sender?.role;
      const path = currentUser?.role === 'admin' 
        ? `/admin/${senderRole}-messages/${conversationId}`
        : `/seller/${senderRole}-messages/${conversationId}`;
